...
import { MOCKED_DIR_FILE, MOCKED_FILE_NAME, MOCKED_QUESTION, MOCKED_VALID_LANGUAGE, MOCKED_NOT_ALLOWED_LANGUAGE } from '../../mocks/constants';
...
describe('AnswerQuestionsHandler', () => {
    ...
    describe('handle', () => {
        ...
        test('have to return the generated call calling using AI', async () => {
            expect(await answerQuestionsHandler.handle(MOCKED_DIR_FILE, MOCKED_FILE_NAME, MOCKED_QUESTION, MOCKED_VALID_LANGUAGE))
        ...
        test('have to validate that filename cannot be empty', () => {
            expect(async () => await answerQuestionsHandler.handle(MOCKED_DIR_FILE, '', MOCKED_QUESTION, MOCKED_VALID_LANGUAGE))
                .rejects
                .toThrow(FilenameCannotBeEmptyError)
        })

        test('have to validate that question cannot be empty', () => {
            expect(async () => await answerQuestionsHandler.handle(MOCKED_DIR_FILE, MOCKED_FILE_NAME, '', MOCKED_VALID_LANGUAGE))
                .rejects
        ...
        test('have to validate that only allow valid languages', async () => {
            await expect(async () => 
                    await answerQuestionsHandler.handle(MOCKED_DIR_FILE, MOCKED_FILE_NAME, MOCKED_QUESTION, MOCKED_NOT_ALLOWED_LANGUAGE)
                )
                .rejects
